...
namespace GuitarConfiguratorSharp.NetCore
{
    public struct Board
    {
        ...
        public static Microcontroller FindMicrocontroller(Board board)
        {
            ...
            else if (UnoBoards.Contains(board) || board.ArdwiinoName == UsbUpload.ArdwiinoName)
            {
                return new Uno(board);
            }
            else if (MegaBoards.Contains(board))
            {
                return new Mega(board);
            }
            else if (Rp2040Boards.Contains(board))
            {
                return new Pico(board);
            }
            else if (MiniBoards.Contains(board))
            {

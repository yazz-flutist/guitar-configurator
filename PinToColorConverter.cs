...
public class PinToColorConverter : IMultiValueConverter
{
    public object? Convert(IList<object?> values, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        if (values[0] is not int || values[1] is not Microcontroller || values[2] is not ConfigViewModel ||
            values[3] is not int) return null;
        ...
        return selectedPin != pin && (microcontroller.GetPin(pin, selectedPin, model.Bindings).Contains("-")) ? Brushes.Red : Brushes.Transparent;
    }
}